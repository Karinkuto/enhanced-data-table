...
function AnimatedGroup({
  ...
  const MotionComponent = React.useMemo(() => {
    ...
    if (typeof as === "function") {
      return motion(as as React.ComponentType<unknown>);
    }
    // fallback
    return motion.div;
  }, [as]);
  const MotionChild = React.useMemo(() => {
    if (typeof asChild === "string" && asChild in motion) {
      // @ts-expect-error: dynamic access
      return motion[asChild];
    }
    if (typeof asChild === "function") {
      return motion(asChild as React.ComponentType<unknown>);
    }
    // fallback
    return motion.div;
  }, [asChild]);
